...
export default function StatusChangeButton({
  ...
  const text = status ? "Disabling" : "Enabling";
  const finalText = `You are ${text} ${title}`;
  const confirmText = status ? "Disable" : "Enable";
  ...
  async function handleStatusChange() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await fetch(`${baseUrl}/api/${statusChangeEndPoint}`, {
          method: "PUT",
        });
        console.log(res);
        const successText = status ? "Disabled" : "Enabled";
        if (res.ok) {
          router.refresh();
          setLoading(false);
          toast.success(`${title} Successfully ${successText}`);
        }
      } else {
        setLoading(false);
      }
    ...
    <>
      {loading ? (
        ...
        <button
          ...
          className={
            status
              ? `font-medium text-red-600 dark:text-red-500 flex items-center space-x-1`
              : `font-medium text-lime-600 dark:text-lime-500 flex items-center space-x-1`
          }
        >
          {status ? (
